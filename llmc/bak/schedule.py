...
class CostModel:
    ...
    def cost(self, op: Op) -> float:
        ...
        if op.type in (OpType.VEC_MATMUL, OpType.MATMUL):
            a = op.attrs
            out_elems = int(a["a_row"]) * int(a["b_col"])
            comp_cycles = out_elems / self.MU_RESULTS_PER_CYCLE
        else:
            # ---- 非 MatMul: 向量单元吞吐率 ----
...
class Scheduler:
    ...
    # schedule.py 中，_allowed_cores 扩展为考虑 HBM PC 负载
    def _allowed_cores(self, op: Op) -> List[int]:
        cores = self.affinity.get(op.type)
        # 如果 op 的输入 tensor 落在某个 PC，优先分配给访问该 PC 延迟低的 core
        # 或者：确保同一 wave 内的 4 个核访问 4 个不同的 PC
    ...
    def schedule(self) -> Schedule:
        ...
        urgency = critical_path(self.g, self.cost) if self.strategy == "critical" \
            else [0.0] * n_ops
