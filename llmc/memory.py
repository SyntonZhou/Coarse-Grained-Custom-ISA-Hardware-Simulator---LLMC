...
class HBMAllocator:
    ...
    def alloc(self, size: int, *, kind: str = "weight", align: int = 64,
              tag: str = "", preferred_pc: Optional[str] = None,
              strict_preferred_pc: bool = False,
              allowed_pcs: Optional[Iterable[str]] = None) -> int:
        """Allocate a tensor in one HBM pseudo-channel.

        Parameters
        ----------
        preferred_pc:
            Preferred PC name or prefix, e.g. "HBM5_PC0" or "HBM5".
        strict_preferred_pc:
            If true, allocation is not allowed to fall back outside the
            preferred PC/prefix.
        allowed_pcs:
            Hard topology constraint.  The allocator may only use these PCs.
            Use this to enforce NMU->PC reachability computed from topology.
        """
        if size <= 0:
        ...
        allowed_set = set(allowed_pcs) if allowed_pcs is not None else None
        order = self._candidate_order(kind, preferred_pc)
        if allowed_set is not None:
            order = [n for n in order if n in allowed_set]
            if not order:
                raise MemoryError(
                    f"no topology-reachable PC for {tag!r} (kind={kind}); "
                    f"allowed_pcs={sorted(allowed_set)}")
        if strict_preferred_pc and preferred_pc:
            pref_allowed = {n for n in self.chan if n.startswith(preferred_pc)}
            if allowed_set is not None:
                pref_allowed &= allowed_set
            order = [n for n in order if n in pref_allowed]
            if not order:
                raise MemoryError(
                    f"preferred PC {preferred_pc!r} for {tag!r} is unavailable "
                    f"under topology constraints")
        for name in order:
            ...
            if addr is not None:
                ...
                return addr
        suffix = f" within allowed PCs {sorted(allowed_set)}" if allowed_set is not None else ""
        raise MemoryError(f"no PC can fit {size} B for {tag!r} (kind={kind}){suffix}")
